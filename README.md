...
Durante a análise do comportamento do Titã, observou-se uma limitação significativa em sua inteligência artificial relacionada à perseguição dos adversários. Quando o jogador se desloca para trás de uma parede ou outro obstáculo, interrompendo a linha de visão direta, o Titã abandona imediatamente a perseguição e permanece inativo até que um novo alvo entre novamente em seu campo de visão. 

Foram implementadas melhorias para tornar o comportamento do Titã mais estratégico, natural e desafiador. O sistema passou a utilizar uma memória da última posição conhecida do jogador, permitindo que o inimigo continue a perseguição mesmo após perder o contato visual, em vez de interromper imediatamente suas ações. Para garantir uma navegação eficiente pelo cenário, foi integrado um sistema de pathfinding baseado no algoritmo A*, utilizando uma grade de navegação que considera obstáculos e margens de segurança para calcular rotas otimizadas. Além disso, foram adicionados mecanismos que impedem perseguições indefinidas, fazendo com que o Titã abandone automaticamente a busca caso o alvo esteja muito distante ou permaneça desaparecido por um longo período, evitando recálculos desnecessários e reduzindo o consumo de processamento. Por fim, foi desenvolvida uma fase de investigação que simula um comportamento mais inteligente: ao chegar à última posição conhecida do jogador sem encontrá-lo, o Titã percorre diversos pontos de busca próximos antes de retornar à patrulha, criando uma sensação de persistência e tornando os confrontos mais imprevisíveis e imersivos.
